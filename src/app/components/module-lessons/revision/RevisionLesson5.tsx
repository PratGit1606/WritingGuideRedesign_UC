...
export default function RevisionLesson5() {
    ...
    <>
              ...
              {items.map((it) => (
                <button
                  ...
                  className={`w-full flex items-start gap-4 p-4 border rounded-lg shadow-sm text-left transition ${
                    completed.includes(it.id) ? "bg-green-50 border-green-300" : "bg-white hover:bg-gray-50"
                  }`}
                  aria-pressed={completed.includes(it.id)}
                >
                  <div
                    className={`w-5 h-5 mt-1 rounded-full border-2 flex items-center justify-center ${
                      completed.includes(it.id) ? "bg-green-500 border-green-500" : "border-gray-400"
                    }`}
                  >
        ...
        <div className="space-y-4">
              ...
              <div className="space-y-3">
                {categories.map((cat) => (
                  <div key={cat.id} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    <div className="bg-amber-100 md:px-4 md:py-2 font-semibold text-center rounded-md">{cat.label}</div>
                      ...
                      <div key={level} className="flex justify-center md:justify-center">
                        <button
                          ...
                          className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition ${
                            skills[cat.id] === level ? "bg-[#A65D3B] border-[#A65D3B]" : "border-gray-300"
                          }`}
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <ol className="list-decimal list-inside space-y-2 text-gray-800 mb-4">
              <li>
                Critically evaluate the effectiveness of your own writing, including organization and the strength of your main argument.
              </li>
      ...
      <LessonFooter
        prev={{
          kind: "section",
          id: "revision-4",
          label: "Go Back to Section 3 of Revision",
        }}
        next={{
          kind: "section",
          id: "editing-1",
          label: "Continue to Editing",
        }}
      />
    </>
  );
